...
namespace EnvoyDelivery
{
    public partial class orders : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            String connString = "datasource=127.0.0.1;port=3306;username=Collins;password=ca4lala3nasa;";
            MySqlConnection conn = new MySqlConnection(connString);
            int id = Convert.ToInt32(Session["id"]);
            MySqlCommand com = new MySqlCommand("SELECT * FROM `envoy`.`products` WHERE `userid` ="+id+" AND `finished`=0 ORDER BY `id` DESC", conn);
            conn.Open();
